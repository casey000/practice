  ...
  <div class="serach-pro-content"  v-title data-title="平台商品">
            ...
            <li class="pro-lists-row f-oh p-lef-rig24" v-for="(item,i) in productLists" :key="i" @click="clickItems(item,i)">
              ...
              <div class="pro-infos dis-inblo">
                ...
                <span class="fontSize-12">商城价：￥{{item.mallPrice}}<span class="fontSize-10 m-left5" style="color: #c6c6c6;">市场价：￥{{item.marketPrice}}</span>
                  <span v-if="item.storeShopStatus == 1" style="float: right;background: #f76d6b;color: white;padding: 0.2rem">已上架</span>
                  <span v-if="item.storeShopStatus == 0" style="float: right;background: #307ff5;color: white;padding: 0.2rem">已下架</span>
                </span>
    ...
    <ul class="bottom-btns fontSize-18 f-oh p-lef-rig24">
      ...
      <li class="btns-choose fontSize-17 f-fr">
        <span @click="delPro">删除</span>
        <span @click="shangjia">上架</span>
        <span @click="xiajiaPro">下架</span>
      </li>
    </ul>
    ...
    data (){
     ...
     methods : {
      ...
      delPro(){
        ...
        this.delProAxios(this.productIdsStr);
      },
       //删除商品
       delProAxios() {
        var vm = this;
         let productIdsStr = vm.productIds.join(',');
        $.post(comLink.yxtHost + '/store/delProduct', {
...
//    	console.log(item)
      ...
      //下架
       xiajiaAxios: function(productIdsStr){
        ...
        axios.get(comLink.yxtHost + '/store/updateStatus', {
          params:{
            'isAll' : vm.searchData.isAll,
            'status' : 0,
            storeId : sessionStorage.weiStoreId,
        ...
        if(this.isRequest){
          ...
          $.ajax({
            ...
            data:{
              ...
              sortBy : vm.searchData.sortDir,
              sortDir : vm.searchData.sortBy,//升序 降序
              openid : sessionStorage.openid,
              storeId : sessionStorage.weiStoreId,
            },
            dataType:'json',
      ...
      //搜索商品列表
      searchMoreAxios: function(){
        // var vm = this;
        // axios.post(comLink.yxtHost + '/product/search', {
        //     keyword : vm.keyWords,
        //     maxPrice :vm.maxPrice,//最高价
        //     minPrice : vm.minPrice,//最低价
        //     pageNum : vm.pageNum,//页码
        //     pageSize : vm.pageSize,//每页显示数
        //     sortBy : vm.sortBy,//	排序字段
        //     sortDir : vm.sortDir,//排序方式
        //     openid : sessionStorage.openid
        //     //type :  不传 默认为非积分商品
        // }).then(function(response) {
        //   if (response.data.code == 1) {
