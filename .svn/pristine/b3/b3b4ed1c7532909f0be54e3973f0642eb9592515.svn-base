<template>
  <div class="vip-content fontSize-12 b-white" v-title data-title="快讯详情">
    <div class="head-main">
      <p class="fontSize-13">{{consult.title}}</p>
      <!--<span class="fontSize-11">{{consult.createDate}}</span>-->
    </div>
    <div class="m-top15 main-text" v-html="consult.content"></div>
  ...
  export default {
    ...
    methods: {
      vipInfosAxios: function () {
        ...
        let id = hash[1];
        $.ajax({
          url: comLink.yxtHost + '/index/consultDetail',
          data: {
            'id': id,
