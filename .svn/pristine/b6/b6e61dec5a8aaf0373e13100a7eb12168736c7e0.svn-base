<template>
  <div class="serach-pro-content"  v-title data-title="平台商品">
    ...
    <ul class="bottom-btns fontSize-18 f-oh p-lef-rig24">
      ...
      <li class="btns-row ruku-instock f-fr" @click="rukuBtn()">
        <span>入库</span>
      </li>
    </ul>
...
//    	console.log(item)
      ...
      //上架
      shangjiaAxios: function(){
        var vm = this;
        // let hash = window.location.hash;
        // hash = hash.split('=');
        // let storeId = parseInt(hash[1]);//传递过来的id
        let productIdsStr = vm.productIds.join(',');
        console.log(productIdsStr)
        axios.get(comLink.yxtHost + '/store/inStore', {
          params:{
            openid :sessionStorage.openid,
            storeId : sessionStorage.weiStoreId,
      ...
      proListsAxios: function(){
        ...
        $.ajax({
          ...
          data: {
            keyword : vm.keyWords,
            categoryId : vm.categoryId,
            maxPrice :vm.searchData.maxPrice,//最高价
            minPrice : vm.searchData.minPrice,//最低价
            pageNum : vm.pageNum,//页码
            pageSize : vm.pageSize,//每页显示数
      ...
      // hideShaixuanPop(){//jquery动画隐藏弹出框
      //   $(".shaixuan-popup").animate({top:'100%'},200);
      //   $('.black-cover').animate({top:'100%'},200);
      //   $('.pop-btns').animate({bottom : '-6.25rem'},200);
      //   console.log('点击了筛选');
      // },
