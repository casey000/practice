<template>
  <div class="order-detail-content" v-title data-title="订单详情">
    ...
    <div class="bottom-btns f-oh p-lef-rig24">
      ...
      <div class="btns f-fr">
        <span @click="confirmCollect" v-if="orderDetail.order.status != 6">确认收货</span>
        <span v-if="orderDetail.order.status == 6">
          <span @click="closeFa2" v-if="orderDetail.order.invoiceStatus == 1">开票中</span>
          <span @click="closeFa2" v-else-if="orderDetail.order.invoiceStatus == 2">查看发票</span>
          <span @click="closeFa()" v-else>开具发票</span>
        </span>
        ...
        $.ajax({
          ...
          success:function (res) {
            if(res.code == 1){
              ...
              if(vm.orderDetail.order.status == 6) {
                vm.disanbu = '已完成';
                vm.activeWord = 3;
              }
            }else{
