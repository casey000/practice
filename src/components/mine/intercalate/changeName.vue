  ...
  <div class="set-weChat-num" v-title data-title="个人信息">
    <div class="set-content">
      <ul class="set-main fontSize-15">
        <li class="set-row">
          <span>真实姓名：</span>
          <input type="text" placeholder="填写您的真实姓名" v-model="realName">
        </li>
      </ul>
    </div>
    <div class="bottom-button" @click="saveRealName()">
      <span>确定</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    //确定按钮
    saveRealName(){
      this.saveRealNameAxios();
    },
    //保存qq
    saveRealNameAxios:function(){
      var vm = this;
      $.ajax({
        url:comLink.yxtHost + '/user/updateUser',
        data:{
          'realityName' : vm.realName,
