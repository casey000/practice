<template>
  <div class="coupon-content-all b-white" v-title data-title="我的优惠券">
      ...
      <ul class="coupon-main">
        <li class="coupon-row m-top10 f-oh f-pr" v-for="(item,i) in couponData" :key="i">
          <div class="left-main f-fl w-color dis-inblo">
            ...
            <p class="fontSize-12" style="color: #333; padding-left: 0.5rem; font-weight: 500;">
          ...
          <div class="right-main f-fl dis-inblo">
            ...
            <p style="font-size: 2rem ;color:#d20404;" class="ellipsis">
            	{{ item.couponName }}
            	<!--{{ item.couponName }}-->
          ...
          <div class="get-coupon fontSize-16 w-color  f-fr" >
            <p class="" @click="clickTitle(item,i)" :class=" { getCoupon : (item.userStatus === 0 || item.userStatus === 1 || item.userStatus === -1) } ">{{item.title}}</p>
          </div>
        </li>
      </ul>
    </div>
    ...
    <!--弹窗-->
    <div class="enter-popup-content" v-if="isShowFirstPop">
    	<div class="close-main" @click="closeHomePop()">
        <i class="icon iconfont icon-close"></i>
      </div>
      <div class="white-part">
        <dt class="top-img">
          <!--<img class="img-pic" :src=homePopObj.logo alt="">-->
        </dt>
        <dt class="title">
          <span class="big-title">恭喜啦</span>
          <span class="small-title fontSize-15">获得商城优惠劵一张</span>
        </dt>
        <dt class="coupon-content f-pr">
          <img class="coupon-img" :src=couponImg alt="">
          <div class="coupon-words f-oh f-abs">
            <div class="left f-fl f-tac">
              <p class="fontSize-13 m-top12" style="color: #e03233;">￥<span class="coupon-price">{{ popCounpon.reduceMoney }}</span></p>
              <p class="m-top10 fontSize-12">{{popCounpon.couponName}}</p>
            </div>
            <div class="right f-fl f-oh">
              <span class="right-details fontSize-12" style="color: #e03233;">{{ popCounpon.couponName }}</span>
              <span class="right-details fontSize-9" style="color: #e03233;">（不含邮费）</span>
              <span class="right-details fontSize-9">{{popCounpon.couponInfo}}</span>
              <div class="right-details fontSize-7">有效期截止：{{popCounpon.validTime}}</div>
            </div>
          </div>
        </dt>
        <dt class="get-yanzhengCode-content m-top5">
          <!--<div class="phone">
            <input class="ipt" type="text"
                   v-model="pop.phoneNumber"
                   @blur="checkPhone()"
                   placeholder="请输入手机号">
          </div>-->
          <div class="yanzheng-code f-oh">
            <input class="ipt f-fl" type="text" placeholder="请输入验证码" v-model="code">
            <span class="get-btn f-fr fontSize-12" @click=" getYanzhenCode()" v-if="show">获取验证码</span>
            <span class="get-btn f-fr fontSize-12" v-if="!show">{{count}}</span>
          </div>
        </dt>
        <dt class="get-coupon-btn m-top10 fontSize-12">
          <span class="btn" @click="getCoupon">领取优惠劵</span>
        </dt>
      </div>
