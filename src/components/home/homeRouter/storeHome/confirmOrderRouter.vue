<template>
  <div  class="confirm-order-content" v-title data-title="确认订单">
    ...
    <div class="view">
      ...
      <div class="pro-content" v-for="(item, index) in orderList.list">
        ...
        <!--每个订单下都有的部分-->
        <div class="itemLists b-white fontSize-15" v-if="index == orderList.list.length-1">
          <ul class="item-list-main p-lef-rig24">
            ...
            <span v-if="orderList.order.type != 1">
      ...
      <div class="finally-pice m-top10 f-pr b-white fontSize-13" v-if="orderList.order.type != 1">
        <dt class="shipping-cost count-price f-oh">
          <div>
            ...
            <span class="title-enough f-fr" @click="showManjianBts()" v-text="manjianTitle">请选择</span>
          </div>
    ...
    <div class="pay-now p-lef-rig24 b-white fontSize-15 f-oh">
      ...
      <dt class="f-fl total-price-acount">总价： ￥{{orderList.order.realAmount}}</dt>
...
//          message:comLink.leaveMsg,
      ...
      getOrderDetail(){
        ...
        $.ajax({
          ...
          success:function (data) {
            if(data.code===1){
              let enoughId = data.data.order.enoughId;
              if (enoughId != undefined && enoughId != null && enoughId != 'undefined' && enoughId != 'null' && enoughId != '') {
                sessionStorage.setItem('enoughId', enoughId);
              }
...
//            console.log(vm.orderList);
              ...
              for(var i = 0,l = lists.length;i < l;i ++){
                ...
                for( var j = 0,jl = lists[i].productList.length;j < jl;j ++){
                  lists[i].productList[j].logo = comLink.yxtImgHost + lists[i].productList[j].logo;
                }
              }
              vm.isRender = true;
