<template>
  <div class="pro-detail-content" v-title data-title="商品详情">
        ...
        <div class="pro-infos f-oh">
          ...
          <div class="fenqi-ways  m-top10 b-white fontSize-15 f-fl">
            ...
            <div class="pro-fenqi-detail p-lef-rig20 fontSize-11">
              <dt class="detail-main f-oh" v-for="(item,i) in secondProduct.periods" :key="i">
                <i class="detail-num w-color f-fl fontSize-6">{{i+1}}</i>
                <p class="detail-word f-fl ">￥{{item}}</p>
              </dt>
            </div>
            <div class="price-main p-lef-rig20 f-oh">
              <dt class="new-price f-fl fontSize-15 m-right15" v-if="productData.mallPrice > 0">秒杀价：￥{{secondProduct.money}}</dt>
      ...
      <div class="popup-main">
        <div class="pro-img-infos f-oh">
          ...
          <div class="pro-word f-fl">
            ...
            <p class="pro-price fontSize-15" >￥{{secondProduct.money}}</p>
          </div>
        </div>
        ...
        <div class="choose-spec">
          <ul class="spec-main fontSize-14">
              ...
              <p class="choose-title f-fl">库存：</p>
               <span class=" f-fl" style="padding: 1rem 0 0 2rem;">{{secondProduct.store}}</span>
            </li>
            ...
            <li class="spec-main-row f-oh">
              ...
              <div class="choose-detail add-reduce-main f-fl f-oh"">
                ...
                <span class="f-fl">{{buyCount}}
                  <!--<input id="number" v-model.number="buyCount" pattern="[0-9]*" type="number" min = "1" readonly/>-->
                </span>
                <!--<p class="add-reduce-btns f-fl">+</p>-->
              </div>
            </li>
    ...
    <div class="buy-btn-content b-white f-oh">
      ...
      <!--开售并未购买过-->
      <div class="daishou" v-if="setTime == '' && secondProduct.hasBuy == 0">
        <div class="daishoubtn" style="width: 100%;text-align: center">
          <span class="" @click="showBuyNow()">立即购买</span>
        </div>
      </div>
      <!--开售并已购买过-->
      <div class="daishou" v-if="setTime == '' && secondProduct.hasBuy == 1">
        <div class="daishoubtn" style="width: 100%;text-align: center">
      ...
      <div class="daishou" v-if="setTime != ''">
        <div class="daishoubtn" style="width: 100%;text-align: center">
          <span class="" style="background-color: grey">暂未开售</span>
        </div>
      </div>
   ...
   export default{
     ...
     methods : {
       ...
       proDetailAxios:function(){
         var _vm = this;
         let proId = window.location.hash.split('=')[1].split('&')[0];
         _vm.productId = proId;
         $.ajax({
       ...
       //查询秒杀商品
       getSecondProduct: function(){
         var vm = this;
         let scId = window.location.hash.split('=')[2];
         $.post(comLink.yxtHost + '/secondTime/getSecondProduct', {
           'scId': scId,
           'openid': sessionStorage.openid
         }, function(resp) {
           if(resp.code == 1){
              vm.secondProduct = resp.data;

             //开售倒计时
             if(vm.secondProduct.countDown != '' && vm.secondProduct.countDown != null) {
               var leftTime = vm.secondProduct.countDown;
               if(leftTime>0){
       ...
       confirmOrderAxios: function(){
         ...
         }else if(this.secondProduct.store == 0){
           Toast({
             //mintui提示效果
