  ...
  <div class="coupon-content-all b-white" v-title data-title="我的优惠券">
    ...
    <div class="coupon-detail">
      <ul class="coupon-main">
        <li class="coupon-row m-top10 f-oh f-pr" v-for="item in couponData">
          ...
          <img src="../../../static/img/yiling.png" class="yiling" v-if="item.useStatus !== null">
          <div class="get-coupon fontSize-16 w-color  f-fr" v-else>
            <p class="" @click="clickTitle(item)">点击领取</p>
          </div>
        </li>
  ...
  export default {
    ...
    methods: {
      ...
      getCouponAxios: function() {
        var vm = this;
        $.ajax({
          url: comLink.yxtHost + '/inside/coupon/getInsideCouponList',
          data: vm.queryData,
          dataType: 'json',
          type: 'post',
          success: function (resp) {
            if (resp.code == 1) {
              vm.couponData = resp.data.list;
            } else {
              Toast({
                message: resp.msg,
                duration: 2000,
      ...
      getAxios: function(data) {
        var _vm = this;
        $.ajax({
          url: comLink.yxtHost + '/inside/coupon/getInsideCoupon',
          data: {
            openid: sessionStorage.openid,
            insideId: sessionStorage.insideId,
            insideCouponId : data.id
          },
          dataType: 'json',
          type: 'post',
          success: function (data) {
