...
//          'base64File' : fr.result.split(',')[1]
      ...
      imageSubmit() {
        ...
        for(var i = 0; i < vm.productList.length; i ++){
          ...
          vm.flag = false;
          $.post(comLink.yxtHost + '/inside/order/commentProduct', {
      ...
      getOrderProductList(){
  		  var vm = this;
        let insideOrderId = vm.$route.query.insideOrderId;
        $.post(comLink.yxtHost + '/inside/order/getProductList', {
          'openid': sessionStorage.openid,
          'insideId': sessionStorage.insideId,
          ...
          if(resp.code == 1){
            ...
            for(var j = 0,jl = vm.productList.length;j < jl;j ++){
              vm.productList[j].productLogo=comLink.yxtImgHost+vm.productList[j].productLogo;
